...
Use `create_census_parquetfiles.ipynb` to read in the very large csv .txt census files and convert them to much smaller .parquet files while also fixing various issues with the raw census data. Use these parquet files for any subsequent analysis etc.